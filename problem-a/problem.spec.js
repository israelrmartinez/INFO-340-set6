...
describe('Song searching page', () => {
  ...
  test('searches on form submission', (done) => {    
    ...
    setImmediate(() => { //wait until promise has resolved (one tick)
      let lastFetchCall = fetchMock.lastCall();
      expect(lastFetchCall).toBeDefined() //fetch should be called on button click 
      expect(lastFetchCall[0]).toMatch(/.*term=TestSearch/); //was called with correct url
      let records = $('#records img');
